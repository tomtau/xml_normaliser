...
public class XNFTransformationTestCourses {
	...
	public void testCreateNewET() {
		List<TransformAction> actions = XNFTransformation.createNewET(0, new FDPath("courses", "courses.course.taken_by.student.@sno"), "courses.course.taken_by.student.name.#PCDATA", originalFds, parsedDTD);
		...
		Assert.assertEquals("newET1", (String) actions.get(1).getParameters()[1]);		
		...
		Assert.assertEquals("newET1", (String) actions.get(4).getParameters()[0]);
		Assert.assertEquals("@eno", (String) actions.get(4).getParameters()[1]);
		...
		Assert.assertEquals(new FDPath("courses.newET0", "courses.newET0.newET1.@sno"), (FDPath) actions.get(6).getParameters()[0]);
		Assert.assertEquals(new FDPath("courses.newET0.newET1"), (FDPath) actions.get(6).getParameters()[1]);
		
		Assert.assertEquals(TransformAction.ActionType.ADD_XFD, actions.get(7).getType());
		Assert.assertEquals(new FDPath("courses", "courses.newET0.newET1.@sno"), (FDPath) actions.get(7).getParameters()[0]);

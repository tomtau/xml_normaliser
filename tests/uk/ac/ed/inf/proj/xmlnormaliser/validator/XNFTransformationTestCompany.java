...
public class XNFTransformationTestCompany {
	...
	public void testMoveAttribute() {
		...
		Assert.assertEquals(TransformAction.ActionType.DELETE_ATTRIBUTE, actions.get(0).getType());
		Assert.assertEquals("employee", (String) actions.get(0).getParameters()[0]);
		Assert.assertEquals("@eno", (String) actions.get(0).getParameters()[1]);
		
		Assert.assertEquals(TransformAction.ActionType.ADD_ATTRIBUTE, actions.get(1).getType());
		Assert.assertEquals("dep_name", (String) actions.get(1).getParameters()[0]);
		Assert.assertEquals("@eno", (String) actions.get(1).getParameters()[1]);
		
		Assert.assertEquals(TransformAction.ActionType.CHANGE_XFD, actions.get(2).getType());
		Assert.assertEquals(new FDPath("company.department.dep_name"), (FDPath) actions.get(2).getParameters()[0]);
		Assert.assertEquals(new FDPath("company.department.dep_name.@eno"), (FDPath) actions.get(2).getParameters()[1]);		
	}
